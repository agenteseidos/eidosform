...
// Pricing da /v4 (fundo branco) — mesma apresentação de selo duplo da v3:
//   • Plus mantém "Mais Popular" (comprador solo)
//   • Professional ganha "Para agências" — a seção de agências aponta pra ele
// Conteúdo vem da FONTE ÚNICA lib/plan-marketing.ts (Fase 2, auditoria LP
// 2026-07-28) — aqui só vive a apresentação (accent/tema claro).
    ...
    <section id="precos" className="py-14 sm:py-24 px-4 sm:px-6 bg-slate-50">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-10">
          ...
          {/* Toggle */}
          <div className="inline-flex items-center gap-1 bg-slate-100 border border-slate-200 rounded-full p-1 sm:p-1.5">
            <button
          ...
          {plans.map((plan) => {
            ...
            return (
              <motion.div
                ...
                className={`relative flex flex-col p-6 rounded-2xl border transition-[box-shadow,border-color] duration-300 ${
                  plan.accent === 'popular'
                    ? 'bg-[#FFFBF2] border-[#F5B731] shadow-xl shadow-[#F5B731]/15 ring-1 ring-[#F5B731]/30 mt-4 sm:mt-0'
                    : plan.accent === 'agency'
                      ? 'bg-[#FBFAFF] border-violet-400 shadow-xl shadow-violet-500/15 ring-1 ring-violet-300/40 mt-4 sm:mt-0'
              ...
              >
                ...
                {plan.accent === 'agency' && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 whitespace-nowrap z-10">
                    <Badge className="bg-violet-600 text-white font-bold border-0 px-3 shadow-lg shadow-violet-500/30">
                      Para agências
                    </Badge>
                  </div>
                )}
                    ...
                    <div>
                      ...
                      {billing === 'annual' && originalPrice !== price && (
                        <p className="text-xs text-slate-500 mt-0.5">
                          ...
                          <span className="text-emerald-600 ml-1">no plano anual</span>
                        </p>
                      )}
                    </div>
                  )}
                ...
                <p
                  className={`text-sm font-bold mb-5 pb-4 border-b border-slate-200 ${
                    plan.accent === 'popular'
                      ? 'text-[#E8923A]'
                      : plan.accent === 'agency'
                        ? 'text-violet-600'
