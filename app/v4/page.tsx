...
import { MockupAgency, MockupSegmentation } from '@/components/v4/section-mockups'
...
const TRAFFIC_FEATURES = [
  ...
  },
  {
    icon: Mail,
    color: BRAND_TILE,
    title: 'O lead chega inteiro no seu e-mail',
    desc: 'Assunto com o nome da pessoa, todas as respostas, a origem da campanha e o horário. Um botão abre a conversa no WhatsApp direto do celular — você decide se vale a ligação sem abrir o painel.',
  },
  {
    icon: BellRing,
    color: BRAND_TILE,
    title: 'Aviso de quem parou no meio',
    desc: 'Começou a responder e não terminou? Você recebe um e-mail com o que já foi preenchido e há quanto tempo parou — a tempo de recuperar o lead que a maioria das ferramentas simplesmente perde.',
  },
  {
    icon: BarChart3,
    color: BRAND_TILE,
...
function BrandThemesCarousel() {
  return <ThemeCarousel variant="light" />
}
...
export default function LandingV4Page() {
  return (
    <div className="landing-display min-h-screen bg-white text-slate-900 overflow-x-hidden">
      ...
      <section className="py-10 px-4 sm:px-6 border-y border-slate-200 bg-slate-50">
        <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-3 gap-3">
          {DIFFERENTIALS.map(({ icon: Icon, text }, i) => (
            <Reveal key={text} index={i} className="flex items-center gap-3 p-3 rounded-xl bg-white border border-slate-200 shadow-sm">
      ...
      {/* Tráfego pago */}
      <section id="trafego-pago" className="py-14 sm:py-24 px-4 sm:px-6 bg-slate-50">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 gap-5">
            {TRAFFIC_FEATURES.map(({ icon: Icon, color, title, desc }, i) => (
              <Reveal key={title} index={i} hoverLift className="group p-7 rounded-2xl bg-white border border-slate-200 shadow-sm hover:shadow-md hover:border-slate-300 transition-[box-shadow,border-color] duration-300">
      ...
      {EMPHASIS_SECTIONS.map(({ id, title, desc, bullets, mockup }, idx) => {
        ...
        return (
          <section key={id} id={id} className={`py-12 sm:py-20 px-4 sm:px-6 ${idx % 2 ? 'bg-slate-50' : ''}`}>
      ...
      <section id="compartilhe" className="py-12 sm:py-20 px-4 sm:px-6">
        <div className="max-w-5xl mx-auto">
          <Reveal className="text-center mb-8 sm:mb-12">
            <h2 className="text-2xl sm:text-4xl font-black leading-tight text-slate-900">Publique onde o seu público está</h2>
          </Reveal>
          <div className="grid sm:grid-cols-3 gap-8 sm:gap-6">
            {SHARE_ITEMS.map(({ icon: Icon, title, desc }, i) => (
              <Reveal key={title} index={i} className="text-center">
                <span className="inline-flex w-11 h-11 rounded-xl bg-[#F5B731]/15 items-center justify-center mb-4">
                  <Icon className="w-5 h-5 text-[#E8923A]" />
                </span>
      ...
      <section id="agencias" className="py-14 sm:py-24 px-4 sm:px-6 bg-slate-50">
        <div className="max-w-5xl mx-auto grid md:grid-cols-2 gap-10 items-center">
          <Reveal>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid sm:grid-cols-3 gap-5 mb-16">
            {FINANCIAL_COMPARISON.map(({ tier, eidos, typeform, ratio }, i) => (
              <Reveal key={tier} index={i} className="p-6 rounded-2xl bg-white border border-slate-200 shadow-sm flex flex-col">
          ...
          {/* Tabela de recursos */}
          <Reveal className="rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
            <div className="grid grid-cols-[1.4fr_1fr_1fr] bg-slate-100 border-b border-slate-200 text-xs sm:text-sm font-bold">
              ...
              <div className="px-2 sm:px-4 py-4 text-[#E8923A] text-center bg-[#F5B731]/10">EidosForm</div>
      ...
      <footer className="border-t border-slate-200 py-12 px-4 sm:px-6 bg-slate-50">
        <div className="max-w-6xl mx-auto">
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-10">
            ...
            <div className="hidden sm:block">
              ...
              <ul className="space-y-1 text-sm text-slate-500">
                <li><a href="#trafego-pago" className="hover:text-slate-900 transition-colors inline-block py-1">Tráfego pago</a></li>
                <li><a href="#comparativo" className="hover:text-slate-900 transition-colors inline-block py-1">vs Typeform</a></li>
                <li><a href="#precos" className="hover:text-slate-900 transition-colors inline-block py-1">Preços</a></li>
                <li><Link href="/login" className="hover:text-slate-900 transition-colors inline-block py-1">Painel</Link></li>
              </ul>
            </div>
            <div className="hidden sm:block">
              <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Suporte</h4>
              <ul className="space-y-1 text-sm text-slate-500">
                <li><a href="#faq" className="hover:text-slate-900 transition-colors inline-block py-1">FAQ</a></li>
                <li><a href="https://wa.me/5583999378937" target="_blank" rel="noopener noreferrer" className="hover:text-slate-900 transition-colors inline-block py-1">Contato</a></li>
                <li><a href={MIGRATION_FORM} className="hover:text-slate-900 transition-colors inline-block py-1">Migração gratuita</a></li>
              </ul>
            </div>
            <div className="hidden sm:block">
              <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Legal</h4>
              <ul className="space-y-1 text-sm text-slate-500">
                <li><Link href="/privacidade" className="hover:text-slate-900 transition-colors inline-block py-1">Privacidade</Link></li>
                <li><Link href="/termos" className="hover:text-slate-900 transition-colors inline-block py-1">Termos de uso</Link></li>
              </ul>
            </div>
          </div>
          <div className="border-t border-slate-200 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-4 text-xs text-slate-400 sm:hidden">
              <Link href="/privacidade" className="hover:text-slate-900 transition-colors">Privacidade</Link>
              <Link href="/termos" className="hover:text-slate-900 transition-colors">Termos de uso</Link>
            </div>
