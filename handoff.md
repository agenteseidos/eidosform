...
# Handoff — Correções Auditoria QA (Reprovada)

**Data:** 2026-05-03  
**Responsável:** Toin  
**Tipo:** Correções QA (reprovado → aprovado pendente re-validação)  
**Status:** ✅ Concluída (commit pushado)

## Demanda
A Zéfa auditou as correções e **REPROVOU**. Ler relatório completo em `auditoria-correcoes-qa.md`.

Corrigir 5 itens críticos:
- P0: TS2322 em `app/api/forms/route.ts(156,5)` — `PixelConfig` vs `null`
- P1: `select('*')` em GET `/api/forms/[id]` — regressão de segurança
- P2: PlanQuotaCard não usa `/api/user/plan-features` + fallback silencioso
- P2: Loop de retry em webhook dispatcher inconsistente com política
- P2: `isSafeUrl` duplicado em múltiplos módulos

## Itens corrigidos

### 1. P0 — TS2322: PixelConfig typing
**Arquivo:** `app/api/forms/route.ts`

- **Problema:** `sanitizedPixels` tinha tipo inferido `{ [x: string]: unknown; ... } | null`, mas `PixelConfig` espera `string | undefined` (nunca `null`)
- **Correção:** Limpar entradas `null` de pixels antes de atribuir. Se todos os valores forem `null`/vazio, `sanitizedPixels = null`
- **Resultado:** `tsc --noEmit` passa sem erros

### 2. P1 — select('*') em GET /api/forms/[id]
**Arquivo:** `app/api/forms/[id]/route.ts`

- **Problema:** `select('*')` expõe automaticamente qualquer coluna nova (risco de exposição futura de dados sensíveis)
- **Correção:** Lista explícita de 39 colunas da tabela `forms` em constante `FORM_COLUMNS`
- **Código morto removido:** `if (!data)` após `.single()` — esse caminho nunca é alcançado

### 3. P2 — PlanQuotaCard: endpoint + erro explícito
**Arquivos:** 
- `components/dashboard/plan-quota-card.tsx` → client component
- `app/api/user/plan-features/route.ts` → adicionar `responses_used`, `responses_limit` à resposta
- `app/(dashboard)/forms/page.tsx` → simplificar (remover query direta)

- **Antes:** Server component, consulta direta `profiles`, fallback silencioso para defaults se erro
- **Depois:** Client component que busca de `/api/user/plan-features` via `useEffect`
  - Loading state while fetching
  - Tratamento explícito de erro: alerta amarelo com botão "Tentar novamente"
  - Endpoint atualizado com lógica de expiração/downgrade automática
  - `quota.responsesUsed` e `quota.responsesLimit` incluídos na resposta

### 4. P2 — Webhook retry loop: 3 retries claros
**Arquivo:** `lib/webhook-dispatcher.ts`

- **Antes:** `delays = [0, 1000, 2000, 4000]`, loop `attempt < 4` — 4 tentativas, mas delays ambíguos
- **Depois:** `MAX_RETRIES = 3`, `retryDelays = [1000, 2000, 4000]`, loop `attempt <= MAX_RETRIES`
  - 1 tentativa inicial + 3 retries com backoff exponencial (1s, 2s, 4s)
  - Comentário corrigido: "FAILED after 3 retries (4 total attempts)"

### 5. P2 — isSafeUrl consolidado
**Arquivos modificados:**
- `lib/schemas/form-schema.ts` → remover duplicata, importar de `lib/html.ts`
- `components/form-player/question-renderer.tsx` → remover duplicata, importar de `lib/html.ts`
- `app/api/forms/route.ts` → importar de `lib/html.ts`, remover duplicata
- `app/api/forms/[id]/route.ts` → importar de `lib/html.ts`, remover duplicata

- **Resultado:** Fonte única de verdade em `lib/html.ts`. Exportada como `export function isSafeUrl(value: unknown): boolean`

## Validação

- ✅ `tsc --noEmit` — **0 erros**
- ✅ Commit: `fix: correções auditoria QA reprovada (P0-P2)`
- ✅ Push: main → 95d1b1a

## Próximo passo

Executar re-validação de QA com Zéfa para confirmar aprovação.
