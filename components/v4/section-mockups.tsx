  ...
  return (
    <div className="select-none" aria-hidden>
      ...
      <div className="mt-4 flex items-center justify-center gap-2">
        {themeList.map((t) => (
          <span key={t.id} className="w-4 h-4 rounded-full border border-slate-300" style={{ backgroundColor: t.primaryColor }} />
        ))}
        <span className="ml-2 text-[11px] text-slate-600">{themeList.length} temas profissionais</span>
      </div>
    </div>
  )
}
