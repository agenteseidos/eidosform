...
function TestimonialCard({ t, index }: { t: Testimonial; index: number }) {
  return (
    <figure className="flex flex-col p-6 rounded-2xl bg-white/[0.04] border border-white/5 hover:border-white/10 transition-all">
      ...
      <div className="flex items-center gap-3 mb-4">
        {t.photo ? (
          <Image
            ...
            width={52}
            height={52}
            className="w-13 h-13 rounded-full object-cover border-2 border-[#F5B731]/40"
          />
        ) : (
          <div
            className={`w-13 h-13 min-w-13 rounded-full bg-gradient-to-br ${AVATAR_GRADIENTS[index % AVATAR_GRADIENTS.length]} flex items-center justify-center text-white font-bold text-lg border-2 border-white/10`}
            aria-hidden
          >
            {initials(t.name)}
          </div>
      ...
      <Quote className="w-4 h-4 text-[#F5B731]/60 mb-2" aria-hidden />
...
export function TestimonialsSection() {
  return (
    <section id="depoimentos" className="py-14 sm:py-24 px-4 sm:px-6 bg-white/[0.02]">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-14">
          <Badge className="mb-4 bg-white/5 text-slate-400 border border-white/10">
            Depoimentos
          </Badge>
          <h2 className="text-3xl sm:text-5xl font-black mb-4">
            Quem usa, <span className="text-[#F5B731]">recomenda</span>
          </h2>
